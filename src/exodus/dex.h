...
private:
    ...
    uint32_t property;
    //! amount desired, in GXX
    int64_t GXX_desired_original;
    int64_t min_fee;
...
public:
    ...
    int64_t getGXXDesiredOriginal() const { return GXX_desired_original; }

    CMPOffer()
      : offerBlock(0), offer_amount_original(0), property(0), GXX_desired_original(0), min_fee(0),
    ...
    CMPOffer(int block, int64_t amountOffered, uint32_t propertyId, int64_t amountDesired,
      ...
      : offerBlock(block), offer_amount_original(amountOffered), property(propertyId),
        GXX_desired_original(amountDesired), min_fee(minAcceptFee), blocktimelimit(paymentWindow),
        txid(tx), subaction(0)
    {
    ...
    CMPOffer(const CMPTransaction& tx)
      : offerBlock(tx.block), offer_amount_original(tx.nValue), property(tx.property),
        GXX_desired_original(tx.amount_desired), min_fee(tx.min_fee),
...
private:
    ...
    int64_t GXX_desired_original;      // copied from the Offer during Accept's creation
...
public:
    ...
    CMPAccept(int64_t amountAccepted, int blockIn, uint8_t paymentWindow, uint32_t propertyId,
      ...
      : accept_amount_remaining(amountAccepted), blocktimelimit(paymentWindow),
        ...
        GXX_desired_original(amountDesired), offer_txid(txid), block(blockIn)
    {
